...
Graph.prototype.dfs = function(to_find, node, graph) {
    ...
    var t_2 = (typeof node === 'string') ? this.getNode(node) : node;
    
    if(t_1 === undefined || t_2 === undefined) return null; 
    if(t_1.name == t_2.name) return t_1;
    
    var edges_out = graph.edgesOut(node);
    var children = [];
    
    for(var i=0; i<edges_out.length; ++i) children.push(edges_out[i].to); 
        
    for(var i = 0; i <children.length; ++i) {
        var child = children[i];
        var found = graph.dfs(to_find, child, graph); 
