...
Graph.prototype.testJSON = function () {
    ...
    return writeJSON({
            "nodes" : [], 
            "edges" : []
        }, 'c:/users/manan/desktop/test.json');
};*/

Graph.prototype.addNode = function(name) {
    var nstr = {"name" : name };
    this.struct.nodes.push(nstr);
};

Graph.prototype.addEdge = function(name, from, to, dt) {
    var nstr = {"name": name,"from": from,"to": to,"data" : dt}
    this.struct.edges.push(nstr);
}
...
function Graph(struct) {
    ...
    var test = validator.validate(this.struct, scheme),
